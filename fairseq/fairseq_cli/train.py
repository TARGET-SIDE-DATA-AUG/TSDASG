...
def main(args):
    ...
    mix_ratio = args.alpha

    while lr > args.min_lr and epoch_itr.next_epoch_idx <= max_epoch :
        # train for one epoch
        # schedule alpha
        # mix_ratio = math.pow((epoch - 55),2) / 3000 + 0.027
        # 1.5 / np.sqrt(np.log(epoch) + 1)
        epoch += 1
        print("MIX_RATIO:",mix_ratio,"EPOCH:",epoch_itr.epoch)
        
        valid_losses, should_stop = train(args, trainer, task, epoch_itr, mix_ratio)
        if should_stop:
            break
...
def train(args, trainer, task, epoch_itr, mix_ratio):
    ...
    for i, samples in enumerate(progress):
        with metrics.aggregate("train_inner"), torch.autograd.profiler.record_function(
            ...
            log_output = trainer.train_step(samples,mix_ratio)
